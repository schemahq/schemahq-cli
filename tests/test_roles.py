...
""")
    ...
    assert len(roles) == 5
    assert other == """CREATE TABLE films (
    code        char(5) CONSTRAINT firstkey PRIMARY KEY,
    title       varchar(40) NOT NULL,
    did         integer NOT NULL,
    date_prod   date,
...
def test_roles():
    ...
    roles, _ = extract_roles("CREATE ROLE george SUPERUSER CREATEDB INHERIT LOGIN REPLICATION BYPASSRLS CONNECTION LIMIT 3;")
    role = roles['george']
    ...
    roles, _ = extract_roles("CREATE ROLE miriam WITH LOGIN PASSWORD 'jw8s0F4' VALID UNTIL '2005-01-01';")
    role = roles['miriam']
